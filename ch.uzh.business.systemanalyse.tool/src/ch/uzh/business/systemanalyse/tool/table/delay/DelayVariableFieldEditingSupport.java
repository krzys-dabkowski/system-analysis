 ...
 *
 */
public class DelayVariableFieldEditingSupport extends EditingSupport{

	private final TableViewer viewer;
	private final int column;
	
	public DelayVariableFieldEditingSupport(TableViewer viewer, int column) {
		super(viewer);
		this.viewer = viewer;
		this.column = column;
	}
	...
	protected CellEditor getCellEditor(Object element) {
		String[] variableValues = Activator.getDefault().getPreferenceStore().getString("DELAY_VALUES").split("v");
	...
	protected Object getValue(Object element) {
		...
		for(int i = 0; i < Activator.getDefault().getPreferenceStore().getString("DELAY_VALUES").split("v").length; i++) {
			if(floatValue == Float.valueOf(Activator.getDefault().getPreferenceStore().getString("DELAY_VALUES").split("v")[i])) {
				return i;
			}
		}
		return 1;
	...
	protected void setValue(Object element, Object value) {
		...
		float floatValue = 9;
		floatValue = Float.valueOf(Activator.getDefault().getPreferenceStore().getString("DELAY_VALUES").split("v")[((Integer)value)]);
		...
		/*((VariableValue)rowDat.getVariableValueArray().get(column 
				- IProjectSettingsConstants.NUMBER_OF_UNEDITABLE_COLUMNS)).setVariableValue(floatValue);*/
	}
}
